...
app.innerHTML = `
  ...
  <section class="hud">
    ...
    <p data-tip>El payaso del rodeo baila automáticamente en cruz: dos pasos en cada dirección y un giro al completar la cruz.</p>
    <ul class="actions">
      <li><strong>Baile en Cruz</strong> Automático</li>
...
// Rotación inicial hacia arriba (norte)
player.rotation.y = -Math.PI / 2
scene.add(player)

// Estado del baile en cruz del payaso del rodeo
const crossDanceState = {
  currentDirection: 0, // 0: arriba, 1: derecha, 2: abajo, 3: izquierda
  stepsInDirection: 0, // Contador de pasos en la dirección actual
  stepSize: 0.3, // Tamaño de cada paso
  stepsPerDirection: 2, // Dos pasos en cada dirección
  isTurning: false, // Si está girando
  turnProgress: 0, // Progreso del giro (0 a 1)
  turnDuration: 0.5, // Duración del giro en segundos
  basePosition: new THREE.Vector3(0, 0, 0) // Posición base del baile
}

// Inicializar HUD
updateHUD()

// Controles del jugador (deshabilitados para el baile automático en cruz)
...
// Actualizar movimiento del payaso en patrón de cruz
const updateCrossDanceMovement = (delta) => {
  const { currentDirection, stepSize, stepsPerDirection, isTurning, turnProgress, turnDuration, basePosition } = crossDanceState
  
  if (isTurning) {
    // Está girando
    crossDanceState.turnProgress += delta / turnDuration
    
    if (crossDanceState.turnProgress >= 1) {
      // Terminó el giro
      crossDanceState.isTurning = false
      crossDanceState.turnProgress = 0
      crossDanceState.currentDirection = (crossDanceState.currentDirection + 1) % 4
      crossDanceState.stepsInDirection = 0
      
      // Actualizar posición base para el nuevo movimiento
      crossDanceState.basePosition.set(player.position.x, 0, player.position.z)
      
      // Rotar el payaso hacia la nueva dirección
      const targetRotation = (crossDanceState.currentDirection * Math.PI / 2) - Math.PI / 2
      player.rotation.y = targetRotation
    } else {
      // Interpolar la rotación durante el giro (90 grados)
      const prevDir = (crossDanceState.currentDirection - 1 + 4) % 4
      const startRotation = (prevDir * Math.PI / 2) - Math.PI / 2
      const endRotation = (crossDanceState.currentDirection * Math.PI / 2) - Math.PI / 2
      ...
      player.rotation.y = startRotation + rotationDiff * crossDanceState.turnProgress
    }
  } else {
    // Está moviéndose en la dirección actual
    const directions = [
      { x: 0, z: -1, rotation: -Math.PI / 2 }, // Arriba (norte)
      { x: 1, z: 0, rotation: 0 }, // Derecha (este)
      { x: 0, z: 1, rotation: Math.PI / 2 }, // Abajo (sur)
      { x: -1, z: 0, rotation: Math.PI } // Izquierda (oeste)
    ]
    
    const dir = directions[currentDirection]
    
    // Mover un paso
    const moveSpeed = stepSize * delta * 4 // Velocidad de movimiento
    const newX = player.position.x + dir.x * moveSpeed
    const newZ = player.position.z + dir.z * moveSpeed
    
    // Calcular distancia desde la posición base
    const distanceFromBase = Math.sqrt(
      Math.pow(newX - basePosition.x, 2) + 
      Math.pow(newZ - basePosition.z, 2)
    )
    ...
    if (distanceFromCenter < maxDistance && distanceFromBase < stepSize * stepsPerDirection) {
      // Puede moverse
      player.position.x = newX
      player.position.z = newZ
      
      // Si completó dos pasos en esta dirección, comenzar giro
      if (distanceFromBase >= stepSize * stepsPerDirection) {
        // Ajustar posición final exacta
        const totalDistance = stepSize * stepsPerDirection
        player.position.x = basePosition.x + dir.x * totalDistance
        player.position.z = basePosition.z + dir.z * totalDistance
        
        // Comenzar giro
        crossDanceState.isTurning = true
        crossDanceState.turnProgress = 0
      }
    } else {
      // Si llegó al límite de la pista o completó los pasos, cambiar de dirección
      if (distanceFromBase >= stepSize * stepsPerDirection || distanceFromCenter >= maxDistance) {
    ...
    // Rotar hacia la dirección de movimiento (solo si no está girando)
    if (!isTurning) {
      player.rotation.y = dir.rotation
    }
  }
}

// Actualizar movimiento del jugador (mantenido por compatibilidad pero no usado)
const updatePlayerMovement = (delta) => {
  // El movimiento ahora es automático con el baile en cruz
  // Los controles manuales están deshabilitados
}
...
const animate = () => {
  ...
  if (!gameState.gameOver) {
    // Actualizar movimiento del payaso en patrón de cruz
    updateCrossDanceMovement(delta)
